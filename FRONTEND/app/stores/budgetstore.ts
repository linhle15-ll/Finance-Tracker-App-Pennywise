...
export const addNewBudget = async () => {
    ...
    try {
        const res = await axios.post('https://pennywise-backend-linhle.vercel.app/add-budget', newFormData , {
...
export const getBudgets = async () => {
    try {
        const res = await axios.get('https://pennywise-backend-linhle.vercel.app/get-budgets');
        if (res.status === 200) {
...
export const deleteBudget = async (id) => {
    try {
        const res = await axios.delete(`https://pennywise-backend-linhle.vercel.app/delete-budget/${id}`);
